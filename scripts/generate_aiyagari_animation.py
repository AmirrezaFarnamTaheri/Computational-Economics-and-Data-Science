...
# --- Animation Function ---
def animate_and_save_distribution(
    g_star, y_trans, a_grid, n_a, n_y, output_path, n_frames=100
):
...
if __name__ == "__main__":
    ...
    for i in range(500):
        V_new, g_star = solve_household_problem_vfi(
            V, a_grid, y_grid, R_STAR, BETA, GAMMA, y_trans
        )
    ...
    if not os.path.exists(output_path):
        animate_and_save_distribution(
            g_star, y_trans, a_grid, N_A, len(y_grid), output_path
        )
    else:
