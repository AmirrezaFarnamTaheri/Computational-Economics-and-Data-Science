...
def generate_convergence_rates():
    ...
    fig, ax = setup_plot(
        "Convergence Rates of Sequences", "Iteration (k)", "Error $|x_k - x^*|$"
    )
    k = np.arange(1, 11)
    ...
    ax.set_ylim(1e-10, 1)
    save_plot(fig, "images/Appendix/convergence_rates.png")
...
def generate_intermediate_value_theorem():
    ...
    ax.legend()
    save_plot(fig, "images/Appendix/intermediate_value_theorem.png")
...
def generate_convex_sets():
    ...
    ellipse = Ellipse(
        xy=(0.5, 0.5), width=0.8, height=0.6, angle=10, color="skyblue", alpha=0.6
    )
    ...
    ax.axis("off")
    save_plot(fig, "images/Appendix/convex_set.png")
    ...
    ax.set_aspect("equal", adjustable="box")
    ax.axis("off")
    save_plot(fig, "images/Appendix/non_convex_set.png")
...
def generate_separating_hyperplane():
    ...
    # Set A
    ellipse_A = Ellipse(
        xy=(0.3, 0.6), width=0.4, height=0.3, color="skyblue", alpha=0.7
    )
    ...
    # Set B
    ellipse_B = Ellipse(
        xy=(0.7, 0.3), width=0.3, height=0.4, color="lightcoral", alpha=0.7
    )
    ...
    y_hyper = -x_hyper + 1.0
    ax.plot(
        x_hyper, y_hyper, "k-", lw=2, label=r"Separating Hyperplane ($p \cdot z = c$)"
    )
    ...
    save_plot(fig, "images/Appendix/separating_hyperplane_theorem.png")
...
def generate_taylor_sin():
    ...
    ax.legend()
    save_plot(fig, "images/Appendix/taylor_sin.png")
...
def generate_gradient_field():
    ...
    plt.tight_layout()
    save_plot(fig, "images/Appendix/gradient_field.png")
...
def generate_comparative_statics():
    ...
    ax.set_ylim(0, 14)
    ax.legend()
    save_plot(fig, "images/Appendix/comparative_statics.png")
...
def generate_normal_plots():
    ...
    ax.text(
        0,
        0.1,
        r"$P(Z \leq 1) = \int_{-\infty}^{1} \phi(z)dz$",
        ha="center",
        fontsize=12,
    )
    save_plot(fig, "images/Appendix/normal_pdf.png")

    # CDF
    fig, ax = setup_plot(
        "Normal Cumulative Distribution Function (CDF)", "z", "Cumulative Probability"
    )
    ax.plot(x, y_cdf)
    ...
    save_plot(fig, "images/Appendix/normal_cdf.png")


def generate_jensen_inequality():
    """Generates a plot illustrating Jensen's Inequality."""
    fig, ax = setup_plot(
        "Jensen's Inequality for a Concave Function", "Wealth (x)", "Utility U(x)"
    )
    ...
    ax.set_ylim(0, 3.5)
    ax.set_xlim(0, 21)
    save_plot(fig, "images/Appendix/jensen_inequality.png")
...
def generate_clt_plots():
    ...
    plot_sum_dist(1, ax1)
    save_plot(fig1, "images/Appendix/clt_1.png")

    fig2, ax2 = setup_plot("", "", "", fig_size=(8, 5))
    plot_sum_dist(2, ax2)
    save_plot(fig2, "images/Appendix/clt_2.png")

    fig3, ax3 = setup_plot("", "", "", fig_size=(8, 5))
    plot_sum_dist(3, ax3)
    save_plot(fig3, "images/Appendix/clt_3.png")
...
def generate_multivariate_normal():
    ...
    save_plot(fig, "images/Appendix/multivariate_normal_distribution.png")
...
def generate_martingale_paths():
    ...
    save_plot(fig, "images/Appendix/martingale_paths.png")
...
def generate_eigenvectors():
    ...
    save_plot(fig, "images/Appendix/eigenvectors.png")
