...
THEORY_TERMS = re.compile(r"theorem|lemma|proposition|proof|deriv|first[- ]order condition|kkt|bellman|equilibrium|asymptotic", re.I)
...
INTERPRETATION = re.compile(r"economic intuition|economic interpretation|policy implication|economically|interpretation", re.I)
...
def main() -> int:
    ...
    for path in sorted(root.rglob("*.ipynb")):
        ...
        markdown = "\n\n".join(source(c) for c in nb.get("cells", []) if c.get("cell_type") == "markdown")
    ...
    rows.sort(key=lambda row: (row["structural_score"], -row["theory_term_hits"], row["path"]))
    (output / "proof_structure_audit.json").write_text(json.dumps({"candidates": rows}, indent=2), encoding="utf-8")
    lines = [
    ...
    lines += ["", "## Review Priority", "", "Notebooks scoring 0–2 should receive the first human proof-depth pass; 3–4 indicates the expected structural ingredients are present, not that the mathematics has been mechanically verified.", ""]
