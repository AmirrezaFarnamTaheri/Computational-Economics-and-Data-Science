...
def animate_and_save_contagion(graph, initial_shock_nodes, output_path):
    ...
    for node in initial_shock_nodes:
        if G.has_node(node):
            G.nodes[node]["status"] = "initially_failed"
        ...
        for failing_node in failing_now:
            ...
            for creditor, _ in G_current_round.in_edges(failing_node):
                if G.nodes[creditor]["status"] == "solvent":
                    G.nodes[creditor]["capital"] -= G_current_round.edges[
                        creditor, failing_node
                    ]["weight"]
    ...
    def update(frame):
        ...
        status_map = {
            "solvent": "skyblue",
            "initially_failed": "orange",
            "failed_in_cascade": "red",
        }
        ...
        ax.set_title(
            f"Financial Contagion - Round: {frame} | Total Failures: {n_failed}",
            fontsize=16,
        )
...
if __name__ == "__main__":
    ...
    if not os.path.exists(output_path):
        ...
        central_node = max(
            dict(G_finance.out_degree(weight="weight")).items(), key=lambda x: x[1]
        )[0]
