...
def generate_cml_sml_plot():
    ...
    portfolio_return = (
        rf
        + (portfolio_std / market_std) * (market_return - rf)
        + 0.05 * (portfolio_std - market_std) ** 2
    )
    ...
    ax1.plot(
        ...
        "b-",
        lw=2,
        label="Efficient Frontier of Risky Assets",
    )
    ...
    ax1.scatter(
        ...
        s=100,
        zorder=5,
        label="Individual Asset (e.g., Stock A)",
    )
    ...
    ax2.scatter(
        [0.0], [rf], color="green", s=150, zorder=5, label="Risk-Free Asset (β=0)"
    )
    ...
    ax2.scatter(
        ...
        s=100,
        zorder=5,
        label="Stock A (Correctly Priced)",
    )
    ax2.scatter(
        [0.8], [0.11], color="purple", s=100, zorder=5, label="Stock B (Undervalued)"
    )
    ax2.scatter(
        [1.5], [0.11], color="orange", s=100, zorder=5, label="Stock C (Overvalued)"
    )
    ...
    save_path = "images/09-Finance/cml_sml_distinction.png"
