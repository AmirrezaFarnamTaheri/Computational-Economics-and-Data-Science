...
def generate_ito_lemma_viz():
    ...
    ax1.plot(
        ...
        markersize=12,
        mew=3,
        label="1st Order Approx. $f(a) + f'(a)dx$",
    )
    ...
    ax2.plot(
        W, W**2, "r-", marker="o", markersize=4, alpha=0.7, label="Stochastic Path"
    )
    ...
    ax2.axhline(
        ...
        ls="--",
        lw=2,
        label=f"E[$f(W_T)$] ≈ {expected_f_WT:.2f} (Ito term)",
    )
    ...
    fig.suptitle(
        "Intuition for Ito's Lemma: Why the Second Derivative Matters",
        fontsize=18,
        y=1.02,
    )
    plt.tight_layout()
