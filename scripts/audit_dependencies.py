...
def main() -> int:
    ...
    out = ROOT / "audit" / "DEPENDENCY_AUDIT.md"
    out.parent.mkdir(exist_ok=True)
    lines = [
        "# Dependency Audit",
        "",
        f"Third-party import modules observed: **{sum(1 for m in imports if m not in sys.stdlib_module_names and m not in local)}**",
        f"Undeclared non-optional imports: **{len(missing)}**",
        "",
        "| Import | Distribution | Occurrences |",
        "|---|---|---:|",
    ]
    ...
    if not missing:
        lines += [
            "",
            "All non-optional notebook imports are represented in the declared dependency surface.",
        ]
    out.write_text("\n".join(lines) + "\n", encoding="utf-8")
