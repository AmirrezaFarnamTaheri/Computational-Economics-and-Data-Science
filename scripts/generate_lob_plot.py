...
def generate_lob_plot():
    ...
    bid_sizes = np.random.randint(100, 1000, len(bid_prices)) * np.exp(
        -np.arange(len(bid_prices)) * 0.5
    )
    bid_sizes = bid_sizes.astype(int)

    # Asks (price ascending)
    ask_prices = np.arange(best_ask, best_ask + 0.10, 0.01)
    ask_sizes = np.random.randint(100, 1000, len(ask_prices)) * np.exp(
        -np.arange(len(ask_prices)) * 0.5
    )
    ...
    ax.axhline(
        best_bid, color="r", linestyle="--", lw=1.5, label=f"Best Bid: ${best_bid:.2f}"
    )
    ax.axhline(
        best_ask, color="g", linestyle="--", lw=1.5, label=f"Best Ask: ${best_ask:.2f}"
    )

    spread = best_ask - best_bid
    ax.annotate(
        f"Bid-Ask Spread: ${spread:.2f}",
