...
def generate_ordered_dict_diagram():
    ...
    dot = graphviz.Digraph(
        "Ordered_Dict_Structure", comment="Python 3.7+ Dict Implementation"
    )
    ...
    # Descriptions
    dot.node(
        "Desc1", "1. Key hash determines index in Indices Array", shape="plaintext"
    )
    dot.node(
        "Desc2",
        "2. Value in Indices Array points to row in Entries Array",
        shape="plaintext",
    )
    dot.node(
        "Desc3",
        "3. Entries are stored densely, preserving insertion order",
        shape="plaintext",
    )
