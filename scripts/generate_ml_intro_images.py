...
def generate_bias_variance_tradeoff_image():
    ...
    for degree in degrees:
        ...
        avg_expected_prediction_error[degree - 1] = np.mean(
            np.mean((y_test_predictions - y_test_true) ** 2, axis=1)
        )
    ...
    ax.plot(
        degrees,
        avg_expected_prediction_error,
        "o-",
        color="red",
        lw=3,
        label="Total Test Error (MSE)",
    )
    ax.axhline(
        noise_variance,
        color="gray",
        ls="--",
        lw=2,
        label="Irreducible Error ($\\sigma^2_{\\epsilon}$)",
    )
