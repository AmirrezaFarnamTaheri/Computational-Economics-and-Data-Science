...
def normalize_badges(nb, filename):
    ...
    for i, cell in enumerate(nb.cells):
        if cell.cell_type == 'markdown':
            ...
            if cell_changed:
                ...
                if not cell.source and i > 0: # If cell became empty and not first cell, maybe mark for deletion?
    ...
    if not nb.cells:
        nb.cells.append(new_markdown_cell(f"# {os.path.splitext(os.path.basename(filename))[0]}\n\n{LICENSE_BADGE}"))
        ...
        if cell0.cell_type == 'markdown':
            ...
            nb.cells.insert(0, new_markdown_cell(f"# {os.path.splitext(os.path.basename(filename))[0]}\n\n{LICENSE_BADGE}"))
...
def ensure_structure_placeholders(nb):
    ...
    for cell in nb.cells:
        if cell.cell_type == 'markdown':
            ...
            if 'the lens' in src and '#' in src: has_lens = True
            if 'learning objectives' in src and '#' in src: has_objectives = True
            if 'prerequisites' in src and '#' in src: has_prereqs = True
            if 'table of contents' in src and '#' in src: has_toc = True
            
    missing = []
    if not has_lens: missing.append("The Lens")
    if not has_objectives: missing.append("Learning Objectives")
    if not has_prereqs: missing.append("Prerequisites")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Standardize notebook headers and badges.")
    parser.add_argument("target", help="File or directory to process")
    parser.add_argument("--fix-badges", action="store_true", default=True, help="Normalize license badges")
