...
def create_hjb() -> bool:
    cells = [
        new_markdown_cell(
            r"""# 08 Continuous-Time Macroeconomics: HJB and Fokker-Planck Methods
...
def create_blp() -> bool:
    cells = [
        new_markdown_cell(
            r"""# 07 BLP Demand Estimation: Random-Coefficients Logit and Share Inversion
...
def create_sdid() -> bool:
    cells = [
        new_markdown_cell(
            r"""# 13 Modern Causal Frontiers: Synthetic Difference-in-Differences and Matrix Completion
...
"""),
    ...
    return write_notebook(
        "06-Econometrics/13_Modern_Causal_Frontiers_SDID.ipynb", cells
    )
...
"""),
    ...
    return write_notebook(
        "08-Time-Series/07_Nonlinear_Time_Series_and_Particle_Filters.ipynb", cells
    )
...
"""),
    ...
    return write_notebook(
        "09-Finance/08_Hawkes_Processes_and_Market_Impact.ipynb", cells
    )
...
def create_dice() -> bool:
    cells = [
        new_markdown_cell(
            r"""# 04 Climate-Macro Integrated Assessment: A DICE-Inspired Laboratory
...
"""),
    ...
    return write_notebook(
        "10-Specialized-Models/04_Climate_Macro_Integrated_Assessment_DICE.ipynb", cells
    )


def append_section(
    rel: str, marker: str, markdown: str, code: str | None = None
) -> bool:
    path = ROOT / rel
    nb = nbformat.read(path, as_version=4)
    if any(
        marker in (c.source if isinstance(c.source, str) else "".join(c.source))
        for c in nb.cells
    ):
        return False
...
def main() -> None:
    ...
    print(
        f"Created {created} frontier notebooks and enriched {enriched} canonical notebooks."
    )
