...
convex_shape = Polygon(
    np.array([[0.1, 0.2], [0.4, 0.8], [0.9, 0.7], [0.8, 0.1], [0.3, 0.1]]),
    closed=True,
...
ax1.text(
    np.mean([p1_c[0], p2_c[0]]),
    np.mean([p1_c[1], p2_c[1]]) + 0.03,
    r"$\theta x + (1-\theta)y$",
    fontsize=12,
)
...
circle2 = Circle(
    (0.8, 0.5), 0.4, color="white", alpha=1.0
)  # White circle to "cut out" a piece
ax2.add_patch(circle1)
ax2.add_patch(circle2)
ax2.add_line(
    plt.Line2D((0.5, 0.8), (0.9, 0.9), color="white", alpha=0)
)  # dummy line to enforce bounds
