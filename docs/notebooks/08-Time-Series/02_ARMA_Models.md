...
> **Source notebook:** [`08-Time-Series/02_ARMA_Models.ipynb`](https://github.com/AmirrezaFarnamTaheri/Computational-Economics-and-Data-Science/blob/main/08-Time-Series/02_ARMA_Models.ipynb)

> Notebook outputs are intentionally omitted from the documentation build; code and narrative remain source-faithful.

# 02 ARMA Models

[![Open in Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/AmirrezaFarnamTaheri/Computational-Economics-and-Data-Science/blob/main/08-Time-Series/02_ARMA_Models.ipynb) [![Launch Binder](https://mybinder.org/badge_logo.svg)](https://mybinder.org/v2/gh/AmirrezaFarnamTaheri/Computational-Economics-and-Data-Science/main?filepath=08-Time-Series/02_ARMA_Models.ipynb) [![Code License: MIT](https://img.shields.io/badge/Code%20License-MIT-yellow.svg)](../LICENSE) [![Content License: CC BY 4.0](https://img.shields.io/badge/Content%20License-CC%20BY%204.0-blue.svg)](https://creativecommons.org/licenses/by/4.0/)
...
## The Lens: Modeling Economic Fluctuations That Don't Trend
**What problem are we solving?**  
Many macro and financial series (inflation, GDP gaps, short-term rates) fluctuate around a stable mean rather than drifting forever. To forecast and to interpret shocks, we need a compact statistical description of *stationary* dynamics—how today relates to yesterday and to past surprises.

**Why this method?**  
AR, MA, and ARMA models give a parsimonious way to capture persistence and short-run dynamics. They also underpin the Box-Jenkins workflow used across policy institutions and central banks. With ACF/PACF diagnostics, we can identify structure, estimate parameters, and validate whether residuals are truly white noise.

**Scope note:** This notebook emphasizes classical, univariate dynamics (AR/MA/ARMA). Modern forecasting workflows and multivariate dynamics (VARs, state-space, IRFs) are covered in Module **08-Time-Series** later notebooks, while Module **06-Econometrics/09_Classical_Time_Series_Analysis.ipynb** provides the econometric foundations.



**Economic question.** In *02 ARMA Models*, what must remain economically invariant when the computational representation changes? Time-series methods exploit dependence across dates, so chronology is part of the data-generating process rather than a nuisance index. The central question is what can be learned from persistence, innovations, and cross-variable dynamics without leaking future information into the past. Model adequacy must therefore be judged through residual diagnostics, stability, and genuinely out-of-sample forecasting or structural restrictions.

### Learning Objectives
* **Distinguish** AR, MA, and ARMA processes and their stationarity/invertibility conditions.
* **Use** ACF/PACF signatures to identify plausible model orders.
* **Estimate** models with `statsmodels` and interpret coefficients economically.
* **Diagnose** residuals using Ljung-Box tests and information criteria.

### Prerequisites
* **Time Series Basics:** Mean, variance, autocorrelation (Module 08 - Introduction).
...
2.  [**Autoregressive (AR) Models**](#2-autoregressive-ar-models)
    - [Simulating and Estimating an AR(2) Process](#simulating-and-estimating-an-ar2-process)
...
4.  [**Autoregressive Moving Average (ARMA) Models**](#4-autoregressive-moving-average-arma-models)
    - [Simulating and Estimating an ARMA(1,1) Process](#simulating-and-estimating-an-arma11-process)
...
![Box-Jenkins Methodology](https://raw.githubusercontent.com/AmirrezaFarnamTaheri/Computational-Economics-and-Data-Science/main/images/png/box_jenkins_methodology.png)
...
```python
display(Markdown(f"> **Note:** Both AIC and BIC correctly identify the true model order as {order_ic.aic_min_order}. We would select the ARMA(1,1) model."))
```
...
try:
    ...
    display(Markdown(f'> **Note:** Successfully downloaded Core CPI data from {start_date} to {end_date}.'))
...
```python
display(Markdown(f"> **Note:** The ADF test p-value is {adf_result[1]:.3f}. If this is low (e.g., < 0.05), we reject the null hypothesis of a unit root, suggesting the series is stationary. Inflation appears to be a very persistent process."))
```
...
## Interactive Lab: AR(1) Persistence

For $y_t=\rho y_{t-1}+\varepsilon_t$, persistence changes both the visual smoothness of the series and the decay rate of the autocorrelation function. Keep the innovation sequence fixed while varying $\rho$ so only the propagation mechanism changes.

```python
try:
    from ipywidgets import FloatSlider, interact

    innovations_widget = np.random.default_rng(123).normal(size=180)

    @interact(rho=FloatSlider(value=0.7, min=-0.95, max=0.98, step=0.05, description="rho"))
    def ar1_widget(rho=0.7):
        y = np.zeros_like(innovations_widget)
        for t in range(1, y.size):
            y[t] = rho * y[t - 1] + innovations_widget[t]
        fig, ax = plt.subplots(figsize=(9, 3.5))
        ax.plot(y, linewidth=1.4)
        ax.set(title=f"AR(1) path with rho={rho:.2f}", xlabel="t", ylabel="y_t")
        plt.show()
except ImportError:
    print("Install ipywidgets to use the AR(1) persistence slider.")
```
