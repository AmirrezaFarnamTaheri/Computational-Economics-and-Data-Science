...
> **Source notebook:** [`07-Machine-Learning/17_Causal_ML.ipynb`](https://github.com/AmirrezaFarnamTaheri/Computational-Economics-and-Data-Science/blob/main/07-Machine-Learning/17_Causal_ML.ipynb)

> Notebook outputs are intentionally omitted from the documentation build; code and narrative remain source-faithful.

# 17 Causal ML

[![Open in Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/AmirrezaFarnamTaheri/Computational-Economics-and-Data-Science/blob/main/07-Machine-Learning/17_Causal_ML.ipynb) [![Launch Binder](https://mybinder.org/badge_logo.svg)](https://mybinder.org/v2/gh/AmirrezaFarnamTaheri/Computational-Economics-and-Data-Science/main?filepath=07-Machine-Learning/17_Causal_ML.ipynb) [![Code License: MIT](https://img.shields.io/badge/Code%20License-MIT-yellow.svg)](../LICENSE) [![Content License: CC BY 4.0](https://img.shields.io/badge/Content%20License-CC%20BY%204.0-blue.svg)](https://creativecommons.org/licenses/by/4.0/)
...
if not DOUBLEML_AVAILABLE: display(Markdown("> **Note:** The 'doubleml' library is not installed (`pip install DoubleML`). Some sections will be skipped."))
if not ECONML_AVAILABLE: display(Markdown("> **Note:** The 'econml' library is not installed (`pip install econml`). Some sections will be skipped."))
```
...
Standard ML optimizes prediction but ignores causality. Policy evaluation requires knowing *treatment effects*, not just correlations. **Causal ML** combines ML's flexibility with causal inference's rigor to estimate heterogeneous treatment effects.

**Why this method?**
Methods like Causal Forests, Double/Debiased ML, and meta-learners allow economists to discover which subgroups benefit most from a policy—moving beyond average treatment effects to personalized policy.



**Economic question.** In *17 Causal ML*, what must remain economically invariant when the computational representation changes? For economists, predictive performance is useful but not sufficient. The model must be evaluated against the decision or forecasting problem, the information set available at prediction time, and the cost of distribution shift or leakage. Ask what inductive bias the method introduces, how tuning choices are validated out of sample, and which errors matter economically. When the goal is causal or structural, prediction should be treated as a nuisance component rather than evidence of identification by itself.

### Learning Objectives
* **Estimate** heterogeneous treatment effects using Causal Forests and meta-learners.
* **Implement** Double/Debiased Machine Learning (DML) for high-dimensional controls.
* **Interpret** Conditional Average Treatment Effects (CATEs) for policy targeting.
* **Validate** causal ML estimates using calibration and honesty checks.

### Prerequisites
* **Causal Inference:** Potential outcomes, ATE, and selection bias (Module 06 - Causal Inference).
* **ML Basics:** Cross-validation and tree-based methods (Module 07 - Intro).
* **Learning-path prerequisite:** [`16_Advanced_Deep_RL.ipynb`](https://github.com/AmirrezaFarnamTaheri/Computational-Economics-and-Data-Science/blob/main/07-Machine-Learning/16_Advanced_Deep_RL.ipynb)

> **Learning path:** Building on [`16_Advanced_Deep_RL.ipynb`](https://github.com/AmirrezaFarnamTaheri/Computational-Economics-and-Data-Science/blob/main/07-Machine-Learning/16_Advanced_Deep_RL.ipynb); next continue with [`18_Natural_Language_Processing.ipynb`](https://github.com/AmirrezaFarnamTaheri/Computational-Economics-and-Data-Science/blob/main/07-Machine-Learning/18_Natural_Language_Processing.ipynb).
...
```python
# Simulate data for the FWL theorem demonstration.
...
```python
display(Markdown(f"> **Note:** Full regression estimate of τ: {tau_full:.4f}"))
```

```python
display(Markdown(f"> **Note:** Residual regression estimate of τ: {tau_resid:.4f}"))
```
...
![Causal Tree Splitting Logic](https://raw.githubusercontent.com/AmirrezaFarnamTaheri/Computational-Economics-and-Data-Science/main/images/07-Machine-Learning/causal_tree_split.png)
...
**1. Mechanism and assumptions (Conceptual):** Explain the loss/objective and inductive bias of **17 Causal ML**. Distinguish optimization error, estimation error, and generalization error in the economic use case.

**2. Reproduce and diagnose (Applied):** Build a leakage-safe validation experiment using Causal Machine Learning: A Practical Guide, On this page. Compare a simple baseline with the featured method using an economically relevant metric and report uncertainty across folds or seeds.
...
- **Object:** *17 Causal ML* turns an economic or statistical question into an explicit mathematical/computational object rather than a black-box routine.
- **Mechanism:** The core sections on Causal Machine Learning: A Practical Guide, On this page, 1. Introduction: Prediction in Service of Inference should be read as a chain from assumptions to equations to executable implementation.
